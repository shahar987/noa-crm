...
function TopProductsWidget({ products, clients, goTo }) {
  const ranked = products
    .map(p => ({ ...p, usage: window.clientsUsing(p.name, clients) }))
    ...
    <Card className="p-6 anim-fade-up" style={{ animationDelay: '120ms' }}>
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <span className="grid place-items-center w-10 h-10 rounded-2xl bg-blush-100 text-blush-600">
        ...
        <button onClick={() => goTo('inventory')} className="text-sm font-semibold text-blush-600 hover:text-berry-700 inline-flex items-center gap-1 transition-colors">
          לקטלוג <Icon name="arrow-left" size={15} />
        </button>
      </div>

      {ranked.length === 0 ? (
        <EmptyState icon="package-search" title="אין מוצרים בקטלוג" sub="הוסיפי מוצרים כדי לראות נתונים." />
      ) : (
        <ul className="flex flex-col gap-3">
          {ranked.map(p => (
            <li key={p.id} className="flex items-center gap-3">
              <span className="grid place-items-center w-9 h-9 rounded-xl flex-none bg-blush-50 text-rosegold-500">
                <Icon name="flower-2" size={17} />
              </span>
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1.5 h-1.5 rounded-full bg-blush-50 overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-l from-blush-400 to-rosegold-400"
...
function Dashboard({ clients, products, goTo }) {
  ...
  return (
    <div data-tab-enter className="flex flex-col gap-6">
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-white/85 text-sm font-medium mb-1">
              <Icon name="sparkles" size={16} /> בוקר טוב, נועה טל-אל
            </div>
          ...
          <div className="flex gap-2.5">
            <Button variant="outline" className="!bg-white/15 !border-white/30 !text-white hover:!bg-white/25" icon="user-plus" onClick={() => goTo('clients')}>לקוחה חדשה</Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <StatCard icon="users" label="לקוחות פעילות" value={totalClients} sub="בספר הלקוחות שלך" accent="blush" delay={40} />
        <StatCard icon="package" label="מוצרים בקטלוג" value={totalProducts} sub="קרמים זמינים לשיוך" accent="gold" delay={90} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3">
          <TopProductsWidget products={products} clients={clients} goTo={goTo} />
        </div>
        ...
        <div className="lg:col-span-2">
          <Card className="p-6 h-full anim-fade-up" style={{ animationDelay: '180ms' }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => goTo('clients')} className="text-sm font-semibold text-blush-600 hover:text-berry-700 inline-flex items-center gap-1 transition-colors">
                לכולן <Icon name="arrow-left" size={15} />
              </button>
            </div>
            <ul className="flex flex-col gap-1">
              {recent.map(c => (
                <li key={c.id}>
                  <button onClick={() => goTo('clients', c.id)} className="w-full flex items-center gap-3 rounded-2xl px-2.5 py-2.5 hover:bg-blush-50 transition text-right">
                    <Avatar name={c.name} size={42} />
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-berry-800 truncate">{c.name}</div>
                      <div className="text-xs text-berry-700/45 flex items-center gap-1">
